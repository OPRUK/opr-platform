...
export async function getFacebookSummary(): Promise<FacebookSummary | null> {
  ...
  try {
    ...
    videosUrl.searchParams.set("fields", "id,title,description,views");
    ...
    if (!accountResponse.ok) {
      console.error("OPR Facebook account query failed", await accountResponse.text());
    }
    if (!insightsResponse.ok) {
      console.error("OPR Facebook analytics query failed", await insightsResponse.text());
    }

    // Film reporting is independent of the page-level Insights metrics. Meta
    // occasionally retires a summary metric; that must not blank every film.
    const account = accountResponse.ok ? await accountResponse.json() : {};
    const insights = insightsResponse.ok ? await insightsResponse.json() : { data: [] };
    ...
    return {
      ...
      films: (videos.data ?? []).map((video: { id?: string; title?: string; description?: string; views?: number }) => ({
        id: video.id ?? "",
