...
export default async function CommunityCooksScreen({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="relative flex flex-1 flex-col overflow-y-auto px-5 pb-8 pt-16">
      ...
      <h1 className="mb-5 text-[27px] font-bold">Families who made this</h1>
          ...
          {cooks.map((cook) => (
            <div key={cook.id} className="flex gap-3 border-b-2 border-[#123C39]/35 py-4">
              {cook.photoUrl ? (
                ...
                <div className="flex h-[52px] w-[52px] flex-shrink-0 items-center justify-center bg-[#EED8B2] text-base font-bold text-[#123C39]">
                  {cook.name.charAt(0).toUpperCase()}
                </div>
              )}
              <div>
                <div className="mb-1 text-base font-bold">{cook.name}</div>
                {cook.note ? <p className="m-0 text-base leading-[1.55] opacity-85">{cook.note}</p> : null}
              </div>
            </div>
          ))}
        </div>
