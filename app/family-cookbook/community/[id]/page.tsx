          ...
          <section className="bg-[#FFF3DF] px-6 py-20">
            <div className="mx-auto max-w-4xl">
              ...
              <ol className="mt-9 space-y-7">
                {recipe.method.split("\n").filter(Boolean).map((step, index) => <li key={step} className="flex gap-6 text-lg leading-8 text-stone-700"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#123C39] text-sm font-bold text-white">{index + 1}</span>{step}</li>)}
              </ol>
            </div>
          </section>
